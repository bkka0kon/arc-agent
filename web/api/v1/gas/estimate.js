// GET /api/v1/gas/estimate — $0.0005 USDC via x402 (settled after upstream success)
...
export default async function handler(req) {
  ...
  return gateAndRun(req, PRICE, DESCRIPTION, async () => {
    ...
    if (!res.ok) return jsonResponse({ error: "rpc_failed", source: "arc-rpc", status: res.status }, 502);
    const json = await res.json();
    if (json.error || !json.result) return jsonResponse({ error: "rpc_error", source: "arc-rpc", detail: json.error }, 502);
